...
(function ($root) {
    ...
    function initEnv() {
        if (!obScriptLoaded)
            // OpenBabel.js not loaded, can not init now, suspend
            return;
        if (typeof Module === "object")
            // module already set up, no need to set up again
            return;
        if (initOps) {
            ...
            if (module && initCallbackName) {
                $root[initCallbackName] =
                    function () // init callback function, 3D generation can only done after wasm runtime is done
                    {
                        runtimeInited = true;
    ...
    function requestExec(func) {
        ...
        if (runtimeInited) execPendingFuncs();
    }
    ...
    function generate2DMolData(molData, options) {
        ...
        if (!obMol) {
            ...
            try {
                ...
                if (obConv.writeToOutput(obMol)) result = obConv.getOutStr();
                else throw "Fail to save generated 3D data";
            } finally {
                obConv["delete"]();
            }
            obMol["delete"]();
