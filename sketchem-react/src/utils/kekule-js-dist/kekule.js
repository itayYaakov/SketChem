(function ($root) {
    if (!$root) {
        if (typeof self === "object") $root = self;
        else if (typeof window === "object" && window && window.document) $root = window;
    ...
    var isInBrowser =
        typeof navigator !== "undefined" && typeof window !== "undefined" && typeof document !== "undefined";
    var hasNodeEnv =
    ...
    if (!isNode) {
        ...
        var docReady = readyState === "complete" || readyState === "loaded" || (readyState === "interactive" && !isIE); // in IE8-10, handling this script cause readyState to 'interaction' but the whole page is not loaded yet
    }
    ...
    function isDocReady() {
        if (!isNode) {
            var readyState = document && document.readyState;
            var isIE = window.attachEvent && !window.opera;
            return readyState === "complete" || readyState === "loaded" || (readyState === "interactive" && !isIE); // in IE8-10, handling this script cause readyState to 'interaction' but the whole page is not loaded yet
        } // no doc
        else return true;
    }
    ...
    function appendScriptFile(doc, url, callback) {
        if (existedScriptUrls.indexOf(url) >= 0) {
            // already loaded
        ...
        if (isNode) {
            ...
            if (callback) {
                if (appendResult === true) callback(null);
                else callback(appendResult); // error
            }
            return appendResult;
        } // browser
        else {
            try {
                ...
                result.onerror = function (error) {
                    if (callback) callback(new Error("Loading script file " + url + " failed"));
                };
    ...
    function loadChildScriptFiles(scriptUrls, forceDomLoader, callback) {
        if (isNode) {
            ...
            if (!isDocReady() && !forceDomLoader) {
                // can directly write to document
                try {
    ...
    var kekuleFiles = {
        lan: {
            files: ["lan/classes.js", "lan/xmlJsons.js", "lan/serializations.js"],
            category: "lan",
            minFile: "root.min.js",
        },
        root: {
            files: ["core/kekule.root.js"],
            category: "root",
            minFile: "root.min.js",
        },
        ...
        webComponent: {
            ...
            files: ["webComponents/kekule.webComponent.base.js", "webComponents/kekule.webComponent.widgetWrappers.js"],
        },
        ...
        calculation: {
            ...
            files: ["calculation/kekule.calc.base.js"],
        },
        ...
        emscripten: {
            ...
            files: ["_extras/kekule.emscriptenUtils.js"],
            category: "extra",
        },
        ...
        inchi: {
            requires: ["lan", "root", "core", "emscripten", "io"],
            files: ["_extras/InChI/kekule.inchi.js"],
            category: "extra",
        },
    };

    var prequestModules = ["lan", "root", "localization", "localizationData", "common"];
    var usualModules = prequestModules.concat([
        "core",
        "html",
        "io",
        "spectroscopy",
    ...
    function analysisEntranceScriptSrc(doc) {
        ...
        if (!loadedByImportOrRequire) {
            ...
            } // use the traditional way, detect each <script> tags
            else {
        ...
        if (matchResult) {
            ...
            if (result.paramStr) {
                ...
                else result.modules = usualModules; // no modules appointed, use default setting

                // handle local data
                if (!result.locals) result.locals = defaultLocals;
                if (result.locals || result.language) {
                    var localNames = [].concat(result.locals || []);
                    if (result.language && localNames.indexOf(result.language) < 0) {
                        // local resources of forced language should always be loaded
                        localNames.push(result.language);
                    }
                    if (localNames.length) {
                        var localizationModuleIndex = result.modules.indexOf("localizationData");
                        if (localizationModuleIndex < 0)
                            // local data module not listed, put local data as first module
                            localizationModuleIndex = -1;
                        for (var i = 0, l = localNames.length; i < l; ++i) {
                            var localName = localNames[i];
                            if (localName === "en")
                                // default local resource, already be loaded, by pass
                                continue;
    ...
    function loadModuleScriptFiles(modules, useMinFile, rootPath, kekuleScriptInfo, callback) {
        // return loaded files
        if (typeof Kekule !== "undefined" && Kekule.LOADED) Kekule.LOADED = false; // a flag, indicating now are loading script files

        var loadedModules = kekuleScriptInfo.modules || [];
        var essentialModules = [];
        for (var i = 0, l = modules.length; i < l; ++i) {
            if (loadedModules.indexOf(modules[i]) < 0) essentialModules.push(modules[i]);
        }
    ...
    function init() {
        ...
        if (isInBrowser) {
            // try get info from <script> tag first
            scriptInfo = analysisEntranceScriptSrc(document);
            var findScriptTag = scriptInfo.src && scriptInfo.path;

            if (findScriptTag) {
                // explicitly use script tag, load files in traditional way
                isNode = false;
            }
            if (!findScriptTag && hasNodeEnv) {
                // dual env
        ...
        if (isNode) {
            ...
            if (!minFileExisted) scriptInfo.useMinFile = false;
        }
        ...
        if (!scriptInfo.singleMinBundle || typeof scriptInfo.singleMinBundle === "undefined") {
            // when loading with a single bundle, no need to load modules
            loadModuleScriptFiles(modules, scriptInfo.useMinFile, scriptInfo.path, scriptInfo, function (error) {
                if (/*isNode*/ typeof exports !== "undefined") {
                    // export Kekule namespace
